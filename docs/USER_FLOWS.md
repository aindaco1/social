...
| ID | Operator outcome and path | UX audit result | Acceptance boundary |
| --- | --- | --- | --- |
| SVC-01 | Open Connections > Provider setup; choose Facebook, Media Staging, X, TikTok, Unsplash, or KLIPY; save credentials/configuration and Active state once. | Fixed. Account and provider setup now share one destination. One explicit save owns Keychain/configuration feedback and preserves existing secrets when blank; the diagnostic matrix is collapsed until needed. | Live credential validity remains a provider gate. |
| SVC-02 | Open setup/docs or copy one field, one provider packet, all setup, or only missing setup without secrets. | Healthy. Copied packets explicitly exclude secret values and direct the operator back to the named Connections tabs. | Automated redaction contracts plus operator review. |
