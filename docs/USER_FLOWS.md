...
Current measured outcomes and priorities live in [Project status](PROJECT_STATUS.md). This inventory owns stable journey IDs and regression coverage, not a second chronological test log. Theme and modal-focus behavior have executable tests in `test/desktop-appearance.test.mjs` and `test/desktop-dialog-focus.test.mjs`.
...
| MEDIA-07 | Enable Local AI Media Labs, probe runtime, preflight, draft alt text, upscale, crop, search, cancel, and review derivatives. | Shipped in 0.1.10. Shared native LiteRT/older-system Wasm session; background work, progress, cancellation before save, preserved originals/alpha, and separate derivatives. | Native-helper offline inference passed on the release. Isolated packaged UI inference/cancel/retry/recovery passed; full-app offline, older-Mac, and human quality acceptance remain open. See [Project status](PROJECT_STATUS.md#evidence-boundaries). |
...
| SYS-04 | Create a local backup containing database, app-owned media, and manifest without Keychain secrets. | Healthy. Scope and latest backup details are visible; long paths wrap. | Isolated packaged backup/restore covered 30 byte-matching media files and derivative metadata. Representative operator data and credentials remain separate acceptance. |
| SYS-05 | Choose a Dust Wave backup, create a safety backup, restore, reload, and reconnect credentials. | Healthy locally. Explicit replacement confirmation, safety backup, readable result paths, database/media totals. | Isolated packaged restores and safety-copy/metadata/hash checks passed. Real credential reconnection and representative production-data migration remain open. |
| SYS-06 | Check the signed feed quietly at launch or on demand; review, explicitly install, and restart. | Fixed. One shared update path, shallow updater resources, and Rust-owned installation/restart. | Official 0.1.9 → 0.1.10 signed update/relaunch passed in CI. Another-Mac data/Keychain survival remains manual. |
...
Use the remaining manual gates above with controlled personal/test accounts and
isolated data. Record the exact signed build, provider response, and visible
result using [Project status](PROJECT_STATUS.md) and the linked acceptance guides.
Publishing 0.1.10 superseded the earlier local-only restriction; it did not verify
all providers or authorize unrelated destructive actions. A rendered screen or
passing source contract is not evidence that a provider accepted an operation.
